    ...
    def train(self, train_list, test_list, output_nodes):
        ...
        for iteration in range(self.epochs):
            for anImage in train_list:
                ...
                # refer to the line number 78 to see why we need this. 
                # This literally creates the image out of the line of the CSV file it just reads, but it's in grey-scaled image so
                # it can be represented in 0-255 floating point values.
                # The input will be shaped as 784 because the input layer has 784 nodes. 
                inputs = (np.asfarray(all_values[1:]) / 255.0 * 0.99) + 0.01
                # create the target output values (all 0.01, except the desired label which is 0.99)
                # The targets will be shaped as 10 because we have 10 digits.
                targets = np.zeros(output_nodes) + 0.01
                # all_values[0] is the target label for this record
                # targets is being mapped to the "number" of the image.
                targets[int(all_values[0])] = 0.99
                output = self.forward_pass(inputs)
                # print("output at: ", output) # output shape at:  (10,)
                # print("output shape at: ", output.shape) # output at:  [0.1826919  0.0559086  0.03234391 0.04904848 0.0153456  0.05501225 0.04385967 0.26981493 0.1310797  0.16489497]
